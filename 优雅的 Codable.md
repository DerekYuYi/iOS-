...
在进行 encoding 和 decoding 时, 需要一个 String 类型的 key 值来保证唯一性. `CodingKey` 协议的作用就是提供这样一个 key 值. 定义遵循 `CodingKey` 协议的枚举, 并申明为 String 类型. 此时你可以定义属性名在 JSON 中对应的唯一 Key 值. 这里属性 `amount` 对应 Key 值是 `loan_amount`. 如果没有特殊申明属性值的 Key 值, 默认为枚举本身对应的 rawValue (String 值). 
