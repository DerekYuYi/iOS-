//
//  RYActivityIndicatorView.swift
//  Spirits
//
//  Created by DerekYuYi on 2019/4/18.
...
@objc protocol RYActivityIndicatorViewDelegate {
    @objc func activityIndicatorView(_ activityIndicatorView: RYActivityIndicatorView,
        circleBackgroundColorAt index: NSInteger) -> UIColor
}

class RYActivityIndicatorView: UIView, CAAnimationDelegate {
    ...
    weak var delegate: RYActivityIndicatorViewDelegate?
...
//    }
    ...
    private func createCircle(_ radius:CGFloat, color:UIColor, posX:CGFloat, posY:CGFloat) -> UIView {
...
//        circle.setTranslatesAutoresizingMaskIntoConstraints(false)
    ...
    private func createAnimation(_ duration:Double, delay:Double) -> CABasicAnimation {
    ...
    private func addCircles() {
        ...
        for i in 0..<numberOfCircles {
            if let colorFromDelegate = delegate?.activityIndicatorView(self, circleBackgroundColorAt: i) {
            ...
            let circle = createCircle(radiusForCircle, color: color, posX: posX, posY: posY)
    ...
    private func updateCirclesanimations() {
        for i in 0..<subviews.count {
            /*
             will be ignored in xcode 7
             */
            let subview = subviews[i]
            subview.layer .removeAnimation(forKey: "scale")
            subview.layer.add(self.createAnimation(duration, delay: Double(i)*delay), forKey: "scale")
        }
    }
...
extension RYActivityIndicatorView {
